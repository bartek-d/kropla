...
TEST(ExtractUtilsTest, extractsMovesFromLB)
{
    const std::string s{
        "(;SZ[6];W[ab];B[df];W[ac];B[ee];W[fc]LB[fc:17][ec:8][dc:1];B[ad]LB[ad:"
        "12][ae:1][af:10];W[ce];B[ed])"};
    const std::string expected_data{
        ".11B.b"
        "......"
        "....C."
    ...
    constexpr int BSIZEX = 6;
    constexpr int BSIZEY = 6;
    const std::vector<int> expected1{1, PLANES, BSIZEX, BSIZEY};
    const std::vector<int> expected2{1, MOVES_USED, BSIZEX, BSIZEY};
    int run = 0;
    std::string actual;
    auto tensorSaver = [&run, &expected1, &expected2, &actual](
                           float* ptr, const std::string& /*filename*/, int a,
                           int b, int c, int d)
    {
        ...
        {
            EXPECT_EQ(expected2, (std::vector<int>{a, b, c, d}));
            for (int move_no = 0; move_no < b; ++move_no)
            {
                const char marker = 'a' + move_no;
                for (int sample = 0; sample < a; ++sample)
                    for (int i = 0; i < c; ++i)
                    {
                        for (int j = 0; j < d; ++j)
                        {
                            if (ptr[sample * b * c * d + move_no * d * c +
                                    i * d + j] >= 0.5f)
                                actual[sample * c * d + i * d + j] =
                                    marker + 'A' - 'a';
                            else if (ptr[sample * b * c * d + move_no * d * c +
                                         i * d + j] >= 0.01f)
                                actual[sample * c * d + i * d + j] = marker;
                        }
                    }
            }
        }
