// TODO: maybe it'd be good to replace std::set and std::map by unordered containers
// TODO: use 'connects' to speed up Threats etc. (DONE in findThreats_preDot)
// TODO: Threats in 2 moves code could be optimised using the fact that each threat2m occurs twice,
//       as a pair (ind0, ind1) and (ind1, ind0)
// TODO: FLAG_SAFE is set and updated, but not in more complicated cases, when the first dot of
//       a threat-in-2-moves is in danger, but also allows to enclose something which cancels the danger
// TODO: Game::engine has to be initialised somewhere
// TODO: new fields opp_thr and opp_threats could be maybe used in other (older) parts of the program
//       to speed up
// TODO: Worm::opp_threats could be used to find Threats::opp_thr faster. On the other hand, Worm::opp_threats
//       are currently unused and could be perhaps deleted.
//
//  ./a.out mc_13.sgf 125 2000  --> dlaczego nie gra (0,3)? dlaczego (0,3) ma takie male prior?
//                                  (problem jest w chooseAnyMove, ktory czesto gra do terytorium)
//  ./a.out mc_16.sgf 130 1000  --> dlaczego brak dame? przez to gra do srodka
// ./asgf80.out mc_20_popr.sgf 274 5   ->> w kontynuacji (16,4) zamyka caly brzuszek - czemu?
//        -> byl blad w getEnclMoves(), poprawiony w board080b.cc, latka uwzgledniona w board082.cc (board081.cc ma ten blad)
//
// blad w descend: rozwijajac liste ruchow powinien uwzgl. t.playouts-prior.playouts>=8 (popr w 081)
//
// blad/klopot w 83:
//   ./aopt83.out mc_23.sgf 231 10000
// czasami zle gra, chyba problemem jest duza liczba dobrych ruchow?
// chyba wybiera dziwne ruchy, np. (13,4), bo amaf je wskazuje jako dobre -- jesli przec. zamknie, to nie da sie zagrac (13,4)
// propozycja: ruchy wewn. stopow dawac jako przegrane do amaf?
//
// blad w 86:
// ./a.out gram_z_v86.sgf 79 100  -- dlaczego nie od razu gra (1,5)?? Aha, bo bierze pod uwage prior.playouts przy wyborze najlepszego ruchu!
//       poza tym (9,9) ma lepsze prior niz (2,5)/(1,5) ==> powod: prior jest ustawiane tylko wg zdobyczy, saved_dots bez znaczenia!
//
// zla gra w 88:
//   ./a.out mc_29.sgf 77 2500
// Znajduje zly ruch (10,12) zamiast (12,12), chyba glownie dlatego, ze po (10,12) nie chce grac (10,11)
// W ruchu 75 z kolei problemem jest chyba to, ze nie widzi, ze czerwony jest zagrozony w 2 ruchach po (11,12)
//
// found errors:
// 017: testowe-sgf/bardzo-duzo-grozb.sgf, move #267/268:
//      After enclosing red dot in #267, both red threats get deleted, although one (larger) remains.
// 021: corrected. There are two test files: testowe-sgf/bardzo-duzo-grozb.sgf  testowe-sgf/znikajace-grozby.sgf
// 072: (corrected in 073): ml_special_moves was cleared in getEnclMoves(), so ml_special_moves was OK only for the
//      first move in generateListOfMoves()
// 093: ./a.out testowe-sgf/zagram190626.sgf 1500 25  --> error in Game::countTerritory_simple(int)
//      ./a.out testowe-sgf/zagram190626.sgf 1500 20 (tez daje blad, ale 19 nie)
// 138: daje blad na: testowe-sgf/mc139-shotcuts-bug2.sgf 1000 500
//
// g++ -g -std=c++11 -Og board.cc
// g++ -g -std=c++11 -O3 -march=native -pthread -DNDEBUG board.cc -o aopt76.out 
// g++ -g -std=c++11 -O3 -pthread -I../../boost/boost_1_63_0 -march=native -DNDEBUG board.cc -o aopt136.out

// g++ -g -O3 -Wa,-aslh -std=c++11 -masm=intel -pthread -DNDEBUG board.cc > board.asm

// testowanie:
// for p in testowe-sgf/*sgf; do echo $p; ./a.out $p; if [ $? -ne 0 ]; then echo "blad"; break; fi; done;

// test szybkosci (i7 6700, 3.4 GHz):
// ./aopt78n.out mc_09.sgf 1000 50000              --> 21347844 mikros  (findBestMove)
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 1    --> 15149080 mikros  Real saved playouts: 34506
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 4    -->  4068809 mikros  Real saved playouts: 32931
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 8    -->  3167853 mikros  Real saved playouts: 34140
// z define SPEED_TEST
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 1    --> 21761322 mikros  Real saved playouts: 50000            ,20074702 mikros za kolejnym razem
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 2    --> 10974061 mikros  Real saved playouts: 50000   [1.98x]  ,10552080 mikros
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 4    -->  6360527 mikros  Real saved playouts: 49989  (root playouts = 50003)   [3.42x]  ,5838200
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 8    -->  4468185 mikros  Real saved playouts: 49928  (root playouts = 50000)   [4.87x]  ,3975097
// Max Frequency without considering Turbo 3508.24 MHz (100.24 x [35])
// Max TURBO Multiplier (if Enabled) with 1/2/3/4 Cores is  40x/39x/38x/37x

// test szybkosci (Athlon II x3 460, 3.4 GHz):    (wersja 78==79 jesli chodzi o szybkosc)
// ./aopt79.out mc_09.sgf 1000 50000               --> 26171666 mikros  (findBestMove)
// z define SPEED_TEST
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 1    --> 27539377 mikros  Real saved playouts: 50000
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 2    --> 13829872 mikros  Real saved playouts: 49999  [1.99x]
// cat mc_09.sgf | ./aopt78n.out - 1000 50000 3    -->  9664288 mikros  Real saved playouts: 49998  [2.85x]

// cat mc_09.sgf | ./aopt86speed.out - 1000 50000 1 --> 25954941 mikros  Real saved playouts: 50000
// cat mc_09.sgf | ./aopt86speed.out - 1000 50000 2 --> 13143933 mikros  Real saved playouts: 50000
// cat mc_09.sgf | ./aopt86speed.out - 1000 50000 3 -->  9421504 mikros  Real saved playouts: 50001

// cat mc_09.sgf | ./aopt95speed.out - 1000 50000 1 --> 28512659 mikros  Real saved playouts: 50000
// cat mc_09.sgf | ./aopt95speed.out - 1000 50000 2 --> 14588593 mikros  Real saved playouts: 50003
// cat mc_09.sgf | ./aopt95speed.out - 1000 50000 3 --> 10269780 mikros  Real saved playouts: 50002

// cat testowe-sgf/zagram299721.sgf | ./aopt95speed.out - 100 15000 1  --> 35573084 mikros  Real saved playouts: 15000
// cat testowe-sgf/zagram299721.sgf | ./aopt95speed.out - 100 15000 3  --> 12271250 mikros  Real saved playouts: 15002

// cat mc_09.sgf | ./aopt96speed.out - 1000 50000 3 --> 9868375 mikros  Real saved playouts: 50002
// cat testowe-sgf/zagram299721.sgf | ./aopt96speed.out - 100 15000 1  --> 34673345 mikros  Real saved playouts: 15000   (34270403 mikros: wersja 97a)
// cat testowe-sgf/zagram299721.sgf | ./aopt96speed.out - 100 15000 3  --> 11819485 mikros  Real saved playouts: 15002

// cat mc_09.sgf | ./aopt97speed.out - 1000 50000 1 --> 27461102 mikros  Real saved playouts: 50002
// cat mc_09.sgf | ./aopt97speed.out - 1000 50000 2 --> 13998500 mikros  Real saved playouts: 50003
// cat mc_09.sgf | ./aopt97speed.out - 1000 50000 3 --> 9918104 mikros  Real saved playouts: 50004
// cat testowe-sgf/zagram299721.sgf | ./aopt97speed.out - 100 15000 1  --> 34075882 mikros  Real saved playouts: 15000
// cat testowe-sgf/zagram299721.sgf | ./aopt97speed.out - 100 15000 3  --> 11671244 mikros  Real saved playouts: 15002

// cat testowe-sgf/zagram299721.sgf | ./aopt103speed.out - 100 15000 1 -->  79056360 mikros  Real saved playouts: 15000
// cat testowe-sgf/zagram299721.sgf | ./aopt103speed.out - 100 15000 1 -->  36074662 mikros  Real saved playouts: 15000 (po wylaczeniu Influence w randomPlayout()
// cat testowe-sgf/zagram299721.sgf | ./aopt103speed.out - 100 15000 1 -->  35072708 mikros  Real saved playouts: 15000 (po wylaczeniu Influence kompletnie)
// cat mc_09.sgf | ./aopt103speed.out - 1000 50000 1 --> 27760571 mikros  Real saved playouts: 50000 (po wylaczeniu Influence kompletnie)

// cat testowe-sgf/zagram299721.sgf | ./aopt107speed.out - 100 15000 1 --> 34772615 mikros  Real saved playouts: 15000
// cat testowe-sgf/zagram299721.sgf | ./aopt107speed.out - 100 15000 3 --> 12126784 mikros  Real saved playouts: 15002
// cat mc_09.sgf | ./aopt107speed.out - 1000 50000 1 --> 27515813 mikros  Real saved playouts: 50002
// cat testowe-sgf/zagram163926.sgf | ./aopt107speed.out - 200 5000 1  --> 21846809 mikros  Real saved playouts: 5000

// v108 z163296: 21641687 mikros;  z299721: 34619939 mikros;  mc_09: 27757297 mikros
// v108+haveConnection,... z163296: 20500045 mikros;  z299721: 33621293 mikros;  mc_09: 27358169 mikros
// v108 ostateczna:
// cat testowe-sgf/zagram299721.sgf | ./aopt108speed_2.out - 100 15000 1 --> 33317102 mikros  Real saved playouts: 15000
// cat mc_09.sgf | ./aopt108speed_2.out - 1000 50000 1 --> 26708328 mikros  Real saved playouts: 50002
// cat testowe-sgf/zagram163926.sgf | ./aopt108speed_2.out - 200 5000 1  --> 20839813 mikros  Real saved playouts: 5000

// cat testowe-sgf/zagram299721.sgf | ./aopt115speed.out - 100 15000 1 --> 33073334 mikros  Real saved playouts: 15000
// cat mc_09.sgf | ./aopt115speed.out - 1000 50000 1 --> 26460716 mikros  Real saved playouts: 50002
// cat testowe-sgf/zagram163926.sgf | ./aopt115speed.out - 200 5000 1  --> 20895739 mikros  Real saved playouts: 5000

// cat testowe-sgf/zagram299721.sgf | ./aopt122speed.out - 100 15000 1 --> 33618971 mikros  Real saved playouts: 15000
// cat mc_09.sgf | ./aopt122speed.out - 1000 50000 1  --> 26358969 mikros Real saved playouts: 50002
// cat testowe-sgf/zagram163926.sgf | ./aopt122speed.out - 200 5000 1  --> 21297564 mikros  Real saved playouts: 5000

// cat testowe-sgf/zagram299721.sgf | ./aopt124speed.out - 100 15000 1 --> 33716366 mikros  Real saved playouts: 15000
// cat mc_09.sgf | ./aopt124speed.out - 1000 50000 1  --> 27204800 mikros Real saved playouts: 50002
// cat testowe-sgf/zagram163926.sgf | ./aopt124speed.out - 200 5000 1  --> 20995397 mikros  Real saved playouts: 5000
// wersja ostatnia:
// cat testowe-sgf/zagram299721.sgf | ./aopt124speed.out - 100 15000 1 --> 33424301 mikros  Real saved playouts: 15000
// cat mc_09.sgf | ./aopt124speed.out - 1000 50000 1  --> 26621507 mikros Real saved playouts: 50002
// cat testowe-sgf/zagram163926.sgf | ./aopt124speed.out - 200 5000 1 --> 21547209 mikros  Real saved playouts: 5000

// v125 (po dodaniu shortcuts do Threat):
// cat testowe-sgf/zagram299721.sgf | ./aopt125speed.out - 100 15000 1 --> 33315888 mikros  Real saved playouts: 15000
// cat mc_09.sgf | ./aopt125speed.out - 1000 50000 1  --> 26859570 mikros Real saved playouts: 50002
// cat testowe-sgf/zagram163926.sgf | ./aopt125speed.out - 200 5000 1 --> 21492698 mikros  Real saved playouts: 5000
//  26355666, 33315148, 21495744  <-- Threat2m::removeMarked()
//  26355246  33170257 21193795  <-- shortcuts as std::array
// 26859440       <-- remove jako zewn. funkcja
// 26660612  33366429  <-- remove tez w threats[].threats
// 26508561  33264222  <-- (usuniete)
//  26512451 33219769 
// ostateczna wersja:  33369298  26405927  21447580

// v126
// cat testowe-sgf/zagram299721.sgf | ./aopt125speed.out - 100 15000 1 --> 32923791 mikros  Real saved playouts: 15000
// cat mc_09.sgf | ./aopt125speed.out - 1000 50000 1  --> 25655392 mikros Real saved playouts: 50002
// cat testowe-sgf/zagram163926.sgf | ./aopt125speed.out - 200 5000 1 --> 21045929 mikros  Real saved playouts: 5000

// profiling:
/*
g++ -g -std=c++11 -O3 -DNDEBUG board.cc -o aopt.out
valgrind --tool=callgrind ./aopt.out testowe-sgf/zagram298719.sgf
python gprof2dot.py -f callgrind callgrind.out.10313 |  dot -Tsvg -o output.svg

lub

perf record  ./aopt.out testowe-sgf/bardzo-duzo-grozb.sgf 1
perf report --stdio
...
/*
 wersja 067
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000)
  1. 4        44 mecze, punkty=39  38 w, 2 r, 4 porazki, 119.6s : 2348s, srednio 2.72s, min-max czas: 1.24--4.14s
  2. MC20000  44 mecze, punkty=5     4w, 2r, 38  porazek ,                 srednio 53.36s, min-max czas: 44.89--60.41s
 11.3%

 wersja 069 (lepsze wspolczynniki w prior)
 4 vs MC 2000, plansza 12x12 (plik 4dgb_vs_MC2000)
  1. 4        52 mecze, punkty=39.5  38 w, 3 r, 11 porazki, 424.1s : 3563s, srednio 8.15s, min-max czas: 4.18--11.88s (wolniejszy, bo bez optym)
  2. MC20000  52 mecze, punkty=12.5  11 w, 3 r, 38 porazek,                 srednio 68.50s, min-max czas: 56.29--86.65s
 24%

 6 vs MC 8000, plansza 12x12 (plik 6_vs_MC8000)
  1. 6       8 meczów, 5w, 1r, 2p, srednio 95.96s (45.55--144.1)
  2. MC8000  8 meczów, 2w, 1r, 5p, srednio 201.7s (156.6--220.2)
 31%

 wersja 071 (Pattern52 --> because of the bug in fact again Pattern3)
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v71)
  1. 4       94 mecze, punkty=68.5  66w, 5r, 23p, srednio 2.66s
  2. MC2000  94 mecze, punkty=25.5  23w, 5r, 66p, srednio 50.64s
 27.1%

 wersja 072 (zmiana wsp. przy wstawianiu do teryt. przeciwnika)
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v72)
  1. 4       114 mecze, punkty=79  73w, 12r, 29p, srednio 2.63s
  2. MC2000  114 mecze, punkty=35  29w, 12r, 73p, srednio 47.56s
 30.7%

 wersja 072
 6 vs MC 8000, plansza 12x12 (plik 6_vs_MC8000_v72)
  1. 6       55 meczow, punkty=31,5  27w, 9r, 19p, srednio 92.42s
  2. MC8000  55 meczow, punkty=23,5  19w, 9r, 27p, srednio 200.9s
 42.7%

 wersja 073 (poprawiony blad w getEnclMoves())
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v73)
  1. 4       38 meczow, punkty=25.5  24w, 3r, 11p, srednio 2.49s
  2. MC2000  38 meczow, punkty=12.5  11w, 3r, 24p, srednio 47.13s
 32.9%

 wersja 074 (dynamic komi, wlaczone Pattern52, zmienione wsp. prior, najlepszy ruch to ten z najw. liczba symulacji)
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v74)
  1. MC2000  290 meczow, punkty=234  221w, 26r, 43p, srednio 48.47s
  2. 4       290 meczow, punkty=56   43w, 26r, 221p, srednio 2.55s
 80.7% (!!!)

 6 vs MC4000, plansza 12x12 (plik 6_vs_MC4000_v74)
  1. MC4000  70 meczow, punkty=59  57w, 4r, 9p, srednio 103.5s
  2. 6       70 meczow, punkty=11   9w, 4r, 57p, srednio 87.05s
 84.3% (!!!)

 wersja 075 (bez zmian w grze, jedyna nowosc: TreenodeAllocator)
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v75)
  1. MC2000  408 meczow, punkty=332,5  315w, 35r, 58p, srednio 48.92s
  2. 4       408 meczow, punkty=75,5    58w, 35r, 315p, srednio 2.58s
 81.5%

 wersja 076 (3 threads, bez dodatkowych zmian, np. bez virtual win/loss)
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v76)
  1. MC2000  174 meczow, punkty=134,5  121w, 27r,  26p, srednio 21.10s
  2. 4       174 meczow, punkty=39,5    26w, 27r, 121p, srednio 2.25s
 77.3%

 4 vs MC 4000, plansza 12x12 (plik 4_vs_MC4000_v76)
  1. MC4000  176 meczow, punkty=150  141w, 18r,  17p, srednio 61.41s  [CPU byl obciazony starymi, niezabitymi procesami]
  2. 4       176 meczow, punkty=26    17w, 18r, 141p, srednio 2.32s
 85.2%

 wersja 077 (jw. + zmienione wsp. dla dame/ruchow wewn. teryt., VIRTUAL_LOSS=3)
  chyba VIRTUAL_LOSS = 3 jest za duze, zmieniam na 1
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v77)  [nie jest wersja ostateczna, po tym poprawilem bledy z niezerowaniem root.t oraz z uakt. komi]
  1. MC2000  340 meczow, punkty=273,5  262w, 23r,  55p, srednio 31.66s  [CPU byl obciazony starymi, niezabitymi procesami]
  2. 4       340 meczow, punkty=66,5    55w, 23r, 262p, srednio 3.11s
 80.4%

 wersja 078/079 (popr. blad, ktory czasami dawal ruch 'dame' wewn. teryt., lepsza obsluga ruchow na bandzie)
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v78)
  1. MC2000  150 meczow, punkty=135  133w, 4r,  13p, srednio 21.46s
  2. 4       150 meczow, punkty=15    13w, 4r, 133p, srednio 2.39s
 90% (!!!)

 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v78)
  1. 4      34 mecze, punkty=26    24w, 4r,  6p, srednio 9.85s
  2. MC500  34 mecze, punkty=8      6w, 4r, 24p, srednio 14.38s
 23.5%

 4 vs MC 20k, plansza 20x20, (plik 4_vs_MC20000_v78_i7_20x20)
  1. MC 20k  35 meczow, punkty=34  34w, 0r, 1p, srednio 822.9s
  2. 4       35 meczow, punkty=1    1w, 0r,34p, srednio 90.36s

 wersja 080 (distance_rave w amafboard, uzywane threats2m w chooseAtariMove())
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v80b)
  1. 4      70 meczow, punkty=42,5    41w, 3r, 26p, srednio 9.56s
  2. MC500  70 meczow, punkty=27,5    26w, 3r, 41p, srednio 16.81s
 39.3%  (znaczna poprawa z v78, ale tez nieco wolniej)

 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v80)
  1. MC2000  116 meczow, punkty=108  107w, 2r,   7p, srednio 21.05s
  2. 4       116 meczow, punkty=8      7w, 2r, 107p, srednio 2.10s
 93.1% (!!!)

 4 vs MC 8000, plansza 20x20, (plik 4_vs_MC8000_v80_20x20)
  1. MC 8000  35 meczow, punkty=32  29w, 0r, 3p, srednio 861.4s
  2. 4        35 meczow, punkty=3    3w, 0r,29p, srednio 162.7s

 wersja 081 (checkBorderMove daje dame, jesli gramy na 1 linii obok uciekajacego przec., prior.playouts wziete pod uwage przy rozwijaniu listy ruchow w generateListOfMoves)
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v81_i7)
  1. MC2000  253 mecze, punkty=231,5  226w, 11r,  16p, srednio 9.99s
  2. 4       253 mecze, punkty=21,5    16w, 11r, 226p, srednio 1.31s
 91.5%

 wersja 082 (poprawiony kolejny blad getEnclMoves())
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v82_i7)
  1. MC2000  224 mecze, punkty=207,5  204w, 7r,  13p, srednio 10.18s
  2. 4       224 mecze, punkty=16,5    13w, 7r, 204p, srednio 1.29s
 92.6%

 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v82_i7)
  1. 4      890 meczow, punkty=522,5   504w, 37r, 349p, srednio 4.85s
  2. MC500  890 meczow, punkty=367,5   349w, 37r, 504p, srednio 10.37s
 41.3%

 wersja 083 (dodane pattern3_value, bez zmian w grze)
  4 vs MC 4000, plansza 20x20, (plik 4_vs_MC4000_v83_20x20)
  1. MC 4000  20 meczow, punkty=18  18w, 0r, 2p, srednio 464.5s
  2. 4        20 meczow, punkty=2   2w , 0r,18p, srednio 173.6s

 wersja 084 (ruchy wewn. zamkniecia przec. daja 2 przegrane do amaf)
 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v84)
  1. MC2000  326 meczow, punkty=310,5  306w, 9r,  11p, srednio 21.79s
  2. 4       326 meczow, punkty=15,5   11w , 9r, 306p, srednio 2.13s
 95.2% (jest poprawa)

 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v84)
  1. 4      500 meczow, punkty=268,5   258w, 21r, 221p, srednio 10.39s
  2. MC500  500 meczow, punkty=231,5   221w, 21r, 258p, srednio 14.08s
 46.3% (jest poprawa)

 6 vs MC 2000, plansza 12x12 (plik 6_vs_MC2000_v84_i7)
  1. MC2000  184 mecze, punkty=161  156w, 10r,  18p, srednio 10.77s
  2. 6       184 mecze, punkty=23    18w ,10r, 156p, srednio 52.05s
 87.5%

 wersja 085 (poprawnie uzywane safety w MC -- wczesniej za bezpieczne uchodzily tylko dolaczone do bandy, rozwoj PossibleMoves -- to bez wplywu)
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v85)
  1. 4      322 meczow, punkty=166,5   160w, 13r, 149p, srednio 10.41s
  2. MC500  322 meczow, punkty=155,5   149w, 13r, 160p, srednio 14.25s
 48.3%

 wersja 086 (uzywane chooseAnyMove_pm)
 wersja b, bez dodawania kar do amaf w terytoriach:
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v86)
  1. MC500  46 meczow, punkty=25   25w, 0r, 21p, srednio 14.37s
  2. 4      46 meczow, punkty=21   21w, 0r, 25p, srednio  9.21s
 54.3% (ale za malo gier!)

 wersja c, z dodawaniem kar do amaf w terytoriach, ale nie dla dame:
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v86c)
  1. MC500  366 meczow, punkty=227   215w, 24r, 127p, srednio 14.00s
  2. 4      366 meczow, punkty=139   127w, 24r, 215p, srednio  8.94s
 62% !!!

 wersja d, z dodawaniem kar do amaf w terytoriach i dla dame:
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v86d)
  1. MC500  208 meczow, punkty=124,5  120w, 9r, 79p , srednio 14.19s
  2. 4      208 meczow, punkty=83,5   79w , 9r, 120p, srednio  8.77s
 59.9% -- hmm...

 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v86d)
  1. MC2000  300 meczow, punkty=290   289w, 2r,  9p, srednio 20.14s
  2. 4       300 meczow, punkty=10      9w, 2r, 289p, srednio 1.89s
 96.7%

 wersja 087 (saved_worms w ThrInfo, do dokladnego liczenia saved_dots; w generateListOfMoves() lepiej ustawiamy prior dla ruchow likwidujacych zagrozenia
             poprzez zamkniecie, bo liczymy saved_dots; prior nieuwzgledniane w wyborze najl. ruchu)
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v87_i7)
  1. MC500  104 mecze, punkty=68,5   66w, 5r, 33p, srednio  8.05s
  2. 4      104 mecze, punkty=35,5   33w, 5r, 66p, srednio  4.94s
 65.9% (jest poprawa)

 wersja 088 (poprawione threats2m: uzywalismy worm zamiast group_id do znajdowania zagrozen na odl. 2; w randomPlayout nie bylo if(dame_moves_so_far>=2) break; )
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v88)
  1. MC500  372 mecze, punkty=248   243w, 10r, 119p, srednio  15.24s
  2. 4      372 mecze, punkty=124   119w, 10r, 243p, srednio  8.66s
 66.6%

 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v88)
  1. MC2000  142 mecze, punkty=139   139w, 0r,   3p, srednio 22.51s
  2. 4       142 mecze, punkty=3       3w, 0r, 139p, srednio 1.76s
 97.9%

 wersja 089: testy z UCB-exploration term (wersja opuszczona)

 wersja 090 (poprawione choosePatternMove, zeby nie wybieral ruchow zagrozonych; poprawione niektore wzorce, szczegolnie kolo bandy)
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v90)
  1. MC500  152 mecze, punkty=116   115w, 2r,  35p, srednio  15.16s
  2. 4      152 mecze, punkty=36     35w, 2r, 115p, srednio   8.51s
 76.3% (duza poprawa!!!)

 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v90_i7)
  1. MC2000  400 meczow, punkty=383   380w, 6r,  14p, srednio 10.15s
  2. 4       400 meczow, punkty=17     14w, 6r, 380p, srednio 1.10s
 95.75%

 2 vs MC500, plansza 12x12 (plik 2_vs_MC500_v90_i7)
  1. MC500   672 mecze, punkty=587,5   583w, 9r,  80p, srednio 5.22s
  2. 2       672 mecze, punkty=84,5     90w, 9r, 583p, srednio 0.03s

 wersja 091 (poprawione AllThreats::addThreat2moves -- nie bylo uaktualniane min_win/min_win2, co ma znaczenie dla chooseAtariMove)
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v91)
  1. MC500  906 meczow, punkty=648   638w, 20r, 248p, srednio  14.89s
  2. 4      906 meczow, punkty=258   248w, 20r, 638p, srednio   8.72s
 71.5% (pogorszenie... (!))

 wersja 092: proba poprawienia liczenia terr_points, opp_dots i singular_dots (wersja opuszczona)
 wersja 093: poprawione liczenie terr_points i opp_dots, ale nie singular_dots (przejete z 092)
 wersja 094: dobrze liczone singular_dots, ale (odkryte) inne klopoty: gdy np. gracz B zamyka cos wewnatrz swojego terytorium w taki sposob,
             ze zamkniecie laczy sie z brzegiem terytorium, ale mimo to nie dzieli tego terytorium na 2 czesci
             Zob. przyklad w komentarzach do Game::findThreats_preDot.
 wersja 095: Poprawione klopoty z zamknieciami wewnatrz wlasnych terytoriow (lub zamkniec).

 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v95)
  1. MC500  220 meczow, punkty=167,5   167w, 1r, 52p , srednio  14.31s
  2. 4      220 meczow, punkty=52,5    52w , 1r, 167p, srednio   8.52s
 76.1% (poprawa w stosunku do 91, tak samo jak 90)
 
 6 vs MC 2000, plansza 12x12 (plik 6_vs_MC2000_v95)
  1. MC2000  482 mecze, punkty=446,5  442w, 9r,  31p, srednio 21.51s
  2. 6       482 mecze, punkty=35,5    31w, 9r, 442p, srednio 67.96s
 92.6% (duza poprawa w stosunku do v84)

 wersja 096: Optymalizacja polegajaca na wolaniu connectionsRecalculate... dla jednego gracza.
 wersja 097: Dalsze optymalizacje connectionsRecalculate...
 wersje 098-101: Influence
 wersja 102: w generateListOfMoves nie dodajemy wins za wplyw bandy

 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v101)
  1. MC500  210 meczow, punkty=133,5   130w, 7r, 73p , srednio  24.16s
  2. 4      210 meczow, punkty=76,5    73w , 7r, 130p, srednio   8.12s
 63.6% (duze pogorszenie)
 
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v102)
  1. MC500  253 mecze, punkty=193,5   192w, 3r, 58p , srednio  24.64s
  2. 4      253 mecze, punkty=59,5    58w , 3r, 192p, srednio   7.84s
 76.5% (powrot do wynikow v95)
 
 wersja 103 bardzo robocza, niezapisana!

 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v103 -- inaczej liczony wplyw, tylko +1 do wins) (103 wersja A, ze stara funkcja isInfluential)
  1. MC500  567 meczow, punkty=403,5   399w, 9r, 159p , srednio  26.59s
  2. 4      567 meczow, punkty=163,5   159w, 9r, 399p, srednio    8.26s
 71.2% (gorzej, byc moze nowy sposob liczenia wplywu wymaga zmiany isInfluential)
 
 ** v103, nowa isInfluential():
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v103b -- nowa funkcja isInfluential)
  1. MC500  102 mecze, punkty=69  68w, 2r, 32p , srednio  36.59s  (obciazony CPU)
  2. 4      102 mecze, punkty=33  32w, 2r, 68p, srednio    8.51s
 67.6% (jeszcze gorzej...)
 
 6 vs MC2000, plansza 12x12 (plik 6_vs_MC2000_v103, rozgrywane na 3 rdzeniach i5-6500 CPU @ 3.20GHz)
  1. MC2000  470 meczow, punkty=431,5   429w, 5r, 36p , srednio  34.61s
  2. 6       470 meczow, punkty=38,5    36w , 5r, 429p, srednio  41.39s
 91.8% (lekkie pogorszenie w stosunku do v95)

 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v103d -- influence wylaczone, InterestingMoves w randomPlayout() -- glownie ciecia)
  1. MC500  244 mecze, punkty=174  172w, 4r, 68p , srednio  13.77s
  2. 4      244 mecze, punkty=70    68w, 4r, 172p, srednio   8.29s
 71.3%, troche lepiej

 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v103e -- influence wylaczone, InterestingMoves w randomPlayout() i prior -- glownie ciecia)
  1. MC500  232 mecze, punkty=177  176w, 2r, 54p , srednio  14.21s
  2. 4      232 mecze, punkty=55    54w, 2r, 176p, srednio   8.45s
 76.3%, troche lepiej
 z wlaczonym influence -- 70.1% (137 meczow),  plik 4_vs_MC500_v103f; 14.96s dla MC500

 4 vs MC 2000, plansza 12x12 (plik 4_vs_MC2000_v103g -- wylaczone influence, interesting moves (cut/connect) w playout i prior)
  1. MC2000  500 meczow, punkty=486   485w, 2r,  13p, srednio 20.43
  2. 4       500 meczow, punkty=14     13w, 2r, 485p, srednio 1.76s
 97.2% (jest poprawa, bylo 95.75% w wersji 90)

 wersja 104:
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v103h -- wersja 104, zmiana: nowe szablony przy bandzie w patt3_symm)
  1. MC500  320 meczow, punkty=243,5  240w, 7r, 73p , srednio  14.54s
  2. 4      320 meczow, punkty=76,5    73w, 7r, 240p, srednio   8.18s
 76.1%, bez zmian

 4 vs MC 2000, plansza 12x12 (dwa pliki 4_vs_MC2000_v104_i5*; i5-6500)
  1. MC2000  705 meczow, punkty=676,5   674w, 5r,  26p, srednio 17.69s
  2. 4       705 meczow, punkty=28,5     26w, 5r, 674p
 96.0% (nieco gorzej, ale podobnie)

 wersja 105: 3x wieksze wagi dla prior (wszystkich)
 4 vs MC 500 (!), plansza 14x14 (!) (plik 4_vs_MC500_v105)
  1. MC500  764 mecze, punkty=627,5  625w, 5r, 134p , srednio  14.14s
  2. 4      764 mecze, punkty=136,5  134w, 5r, 625p, srednio   7.50s
 82.1%, jest poprawa

 3 vs MC 1000, plansza 16x16 (plik 3_vs_MC1000_v105)
  1. MC1000  200 meczow, punkty=191,5  191w, 1r, 8p  , srednio  81.86s
  2. 3       200 meczow, punkty=8,5      8w, 1r, 191p, srednio   3.95s
 95.75%

 wersja 106: optymalizacja findEnclosure: specjalny kod dla zamkniec na 2 kropki oraz sprawdzanie szybkiej mozliwosci ucieczki
 wersja 107: minimalna optymalizacja: zwiekszenie sizeof(Coord::nb8) do 32, zeby nie trzeba bylo robic np. nb8[(i+6) & 7]

 4 vs MC 2000, plansza 20x20 (pliki 4_vs_MC2000_20x20_v107, 4_vs_MC2000_20x20_v107_i5a, 4_vs_MC2000_20x20_v107_i5b)
  1. MC2000  48 meczow, punkty=47  47w, 0r,  1p, srednio  322.4s
  2. 4       48 meczow, punkty=1    1w, 0r, 47p, srednio  228.9s
 na i5 (lacznie)
  1. MC2000  170 meczow, punkty=157  157w, 0r,  13p, srednio  180.4s
  2. 4       170 meczow, punkty=13    13w, 0r, 157p, srednio  102.4s
 lacznie: 204/218 = ok. 93.6%

 wersja 108: usprawnienia findThreats2moves_preDot: findClosableNeighbours, haveConnection, ulepszone findThreats2moves_preDot__getRange
             poprawiony drobny blad w czesci '2 dots in the neighbourhood of [ind] to make an enclosure' funkcji findThreats2moves_preDot
              -- nie znajdowalo niektorych zagr, a niektore znajdowalo podwojnie (stara wersja po poprawieniu zostala zakomentowana,
                 bo nowsza jest szybsza)
            Przyspiesza ok. 3-5% w stosunku do v107 (wiecej na duzych, mniej na malych planszach).

 wersja 109: usprawnienia findEnclosure: zamiana findNextOnRight na findDirectionNo.
 wersja 110: 'cost' moves/floodFill

 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_v110)
  1. MC500  1555 meczow, punkty=1285  1276w, 18r,  261p, srednio  22.93s
  2. 4      1555 meczow, punkty=270    261w, 18r, 1276p, srednio   7.60s
 82.6%, w zasadzie tak samo, a 1.5x wolniej :(

 wersja 111-112: 'cost' ograniczone do glebokosci pierwszych 2 ruchow w generateListOfMoves
  4 vs MC 500, plansza 12x12 (!) (plik 4_vs_MC500_v112)
  1. MC500  3941 meczow, punkty=3343,5  3294w, 99r, 548p , srednio  9.83s
  2. 4      3941 meczow, punkty=597,5    548w, 99r, 3294p, srednio  2.16s
 84.8%, troche lepiej niz wczesniej na 14x14, ale nie wiadomo, czy tak samo byloby na 14x14

  4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v112, grane na 3 rdzeniach i5)
  1. MC500  1879 meczow, punkty=1532,5  1521w, 23r, 335p , srednio  14.53s
  2. 4      1879 meczow, punkty=346,5    335w, 23r, 1521p, srednio  4.52s
 81.6%, troche gorzej (!)
   w drugim pliku (4_vs_MC500_14x14_v112b, grane na 3 rdzeniach i5)
  1. MC500  1868 meczow, punkty=1559  1547w, 24r, 297p , srednio  14.58s
  2. 4      1868 meczow, punkty=309    297w, 24r, 1547p, srednio  4.63s
 83.5%, troche lepiej (!)
 srednio 82.5%, tak samo...

 wersja 113: w choosePatternMove() wybieramy czasami skok o 1;
             wiele wersji roboczych, ostatecznie wylaczone 'cost' calkowicie
  4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v113e)
  1. MC500  717 meczow, punkty=598,5  595w, 7r, 115p , srednio  14.81s
  2. 4      717 meczow, punkty=118,5  115w, 7r, 595p , srednio  7.89s
 83.4%  (moze nieco lepiej)

 wersja 114: dodajemy do AMAF przegrane za gre na polach, na ktorych nie byloby sensu grac,
             bo przeciwnik natychmiast by postawiona kropke mogl zamknac (bez straty teryt);
             Do tego wykorzystujemy bit HISTORY_TERR.
             Ogolnie staty sie chyba nieco pogarszaja, ale lepiej wyglada sytuacja w pozycjach
             typu './aopt114.out mc35-nie-gra-sieci.sgf 90 500 1' i z tego wzgledu to zostawiam.

  4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v114c) -- wersja wstepna, kara za kazde postawienie do srodka, rowniez ze strata terr (chyba niedobre, bo to moga byc redukcje)
  1. MC500  239 meczow, punkty=191  190w, 2r, 47p , srednio  14.68s
  2. 4      239 meczow, punkty=48    47w, 2r, 190p, srednio  8.24s
 79.9%  (duzo gorzej)
 
  4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v114d) -- wersja ostateczna
  1. MC500  106 meczow, punkty=82,5  82w, 1r, 23p , srednio  28.22s (cpu zaladowane...)
  2. 4      106 meczow, punkty=23,5  23w, 1r, 82p, srednio  7.59s
 77.8%  (duzo gorzej, ale tez malo gier)
 
 wersja 115: zmiany w chooseAtariMove: wieksze zdobycze maja wieksze p-stwa
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v115)
  1. MC500  2252 mecze, punkty=1820  1807w, 26r, 419p , srednio  14.12s
  2. 4      2252 mecze, punkty=432    419w, 26r, 1807p, srednio  7.72s
 80.8% (nieco gorzej niz v112 i poprzednie)

 4 vs MC 2000, plansza 12x12  (plik 4_vs_MC2000_v115, 3 rdzenie i5)
  1. MC2000  334 mecze, punkty=327  326w, 2r, 6p  , srednio  17.10s
  2. 4       334 mecze, punkty=7      6w, 2r, 326p, srednio   1.28s
 97.9% (vs 96.0% w v104)

 6 vs MC 2000, plansza 12x12  (plik 6_vs_MC2000_v115, 3 rdzenie i5)
  1. MC2000  107 meczow, punkty=99,5  99w, 1r, 7p , srednio  18.01s
  2. 6       107 meczow, punkty=7,5    7w, 1r, 99p, srednio  40.57s
 93% (vs do tej pory najlepsza v95: 92.6%)

 4 vs MC 2000, plansza 20x20  (plik 4_vs_MC2000_20x20_v115, 3 rdzenie i5)
  1. MC2000  20 meczow, punkty=19  19w, 0r, 1p , srednio  172.4s
  2. 4       20 meczow, punkty=1    1w, 0r, 19p, srednio  128.2s
 95%, ale za malo gier

 wersja 116: uzycie boost::small_vector, znajdowanie dodatkowych grozb w 2 ruchach (jeden ruch obok kropki, drugi daleko), zmiany w patt3_symm,
             zeby nie uwazal ciecia bambusa za ciekawy ruch
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v116, jeszcze bez zmian w patt3_symm)
  1. MC500  200 meczow, punkty=149,5  149w, 1r, 50p , srednio  14.48s
  2. 4      200 meczow, punkty=51,5    50w, 1r, 149p, srednio  7.52s
 74.8% (duzo gorzej!! :(  )

 6 vs MC 2000, plansza 12x12  (plik 6_vs_MC2000_v116, 3 rdzenie i5)
  1. MC2000  390 meczow, punkty=370,5  369w, 3r, 18p , srednio  18.32s
  2. 6       390 meczow, punkty=19,5    18w, 3r, 369p, srednio  39.30s
 95% (vs do tej pory najlepsza v115: 93%)

 wersja 117: optymalizacja Game::possibleMoves_updateSafetyDame()
 wersja 118: threats2m uzywane rowniez w generateListOfMoves
 6 vs MC 2000, plansza 12x12  (plik 6_vs_MC2000_v118, Athlon x3)
  1. MC2000  614 meczow, punkty=590  587w, 6r, 21p , srednio  21.34s
  2. 6       614 meczow, punkty=24    21w, 6r, 587p, srednio  67.37s
 96.1% (drobna poprawa)

 wersja 119: poprawiony blad w Pattern52::generateFromStr(), dawal atomy z atari, ktore nie byly rozpoznawane; wlaczone patt52_edge w generateListOfMoves()
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v119)
  1. MC500  229 meczow, punkty=177  175w, 4r, 50p , srednio  13.79s
  2. 4      229 meczow, punkty=52    50w, 4r, 175p, srednio  8.01s
 77.3% (nieco lepiej niz v116, ale gorzej niz starsze wersje)
 
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v119b, MC_SIMS_EQUIV_RECIPR = 1.0 / 400.0)
  1. MC500  1106 meczow, punkty=893,5  885w, 17r, 204p, srednio  14.04s
  2. 4      1106 meczow, punkty=212,5  204w, 17r, 885p, srednio  7.16s
 80.8% (lepiej)

 wersja 120: proba lazy evaluation dla threats2m wewnatrz terytorium -- porzucona, bo potencjalne przyspieszenie jest minimalne
 wersja 121: ulepszone addClosableNeighbours, znajduje nieco mniej sasiadow
 wersja 122: w checkThreats2moves_postDot sprawdzamy, czy zamkniecie bedzie (prawdopodobnie) tylko jedno, i jesli tak,
             to szukamy najpierw (jednego) simpleEncl, a ew. potem (jednego) nonSimpleEncl.
             Ponadto uzycie konstruktora Enclosure z std::move w findSimpleEnclosure.
 wersja 123: W findThreats2moves_preDot(), w czesci dla dwoch kropek, jednej kolo ind, drugiej kolo pierwszej, sprawdzamy
             dodatkowo, czy pierwsza nie sasiaduje z grupa sasiadujaca z ind. Jesli tak, to mozliwe, ze nasze thr2m jest w istocie
             zwyklym zagr w 1 ruchu. Troche przyspiesza, ale chyba tez poprawia jakosc gry, przynajmniej w 'mc35-nie-gra-sieci.sgf 40 15000 1'
             * Zdalem sobie sprawe, ze takie sprawdzanie wyklucza wiele waznych zagrozen, np. drugie tutaj:
	     //   . . . . . .              . . . . . .
	     // .             .          .             .
	     // .         L   .          .             .
	     // .       .   0              .           0
 ...
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v123)
  1. MC500  1300 meczow, punkty=1056  1051w, 10r, 239p , srednio  14.97s
  2. 4      1300 meczow, punkty=244    239w, 10r, 1051p, srednio  7.53s
 81.2% (zaniedbywalnie lepiej)

 wersja 124: Rozne poprawki threats2m: poprzednie usprawnienie bylo niepoprawne, dodane deleteMiai() (po kasowaniu calej galezi zagrozen
             globalne is_in_2m_miai nie bylo wcale uaktualniane).
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v124)
  1. MC500  1749 meczow, punkty=1425  1421w, 8r, 320p , srednio  13.66s
  2. 4      1749 meczow, punkty=324    320w, 8r, 1421p, srednio  7.42s
 81.5% (zaniedbywalnie lepiej)

 wersja 125: Optymalizacje zwiazane z removeMarked (zamiana usuwanego elementu z ostatnim) oraz z shortcuts (zapamietywanie shortcuts w Threat).
 wersja 126: Rozne drobne optymalizacje.
 wersja 127: Zmiany w generateListOfMoves: inne (zwykle wieksze) wspolczynniki prior, dodatkowo kary za is_in_encl2/miai;
             poprawiona pomylka: przy okrazeniach t.prior nie zawsze bylo ustawiane na aktualna wartosc i czasami niektore prior liczyly sie do symulacji.
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v127)
  1. MC500  1128 meczow, punkty=945  943w, 4r, 181p, srednio  12.68s
  2. 4      1128 meczow, punkty=183  181w, 4r, 943p, srednio  6.98s
 83.8% (lepiej)

 6 vs MC 2000, plansza 12x12  (plik 6_vs_MC2000_v127, Athlon x3)
  1. MC2000  1665 meczow, punkty=1597,5  1592w, 11r, 62p  , srednio  18.44s
  2. 6       1665 meczow, punkty=67,5    62w  , 11r, 1592p, srednio  72.86s
 95.9% (drobne pogorszenie w stosunku do v118)

 wersja 128: Powrot do wspolczynnikow w generateListOfMoves z v126, ale zostaly z v127: kary za is_in_encl2/miai oraz
             poprawa pomylki: przy okrazeniach t.prior nie zawsze bylo ustawiane na aktualna wartosc i czasami niektore prior liczyly sie do symulacji.
             (wersja opuszczona!)
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v128)
  1. MC500  297 meczow, punkty=241  239w, 4r, 54p, srednio  13.29s
  2. 4      297 meczow, punkty=56   54w , 4r, 239p, srednio  7.35s
 81.1% (jednak gorzej, wiec poprawa w 127 nie wynikala z poprawek bledow, tylko z innych wspolczynnikow)

 wersja 129: powrot do 127.
 wersja 130: nowa klasa WildcardReplacer do tworzenia szablonow (znacznie przyspieszyla poczatkowa inicjalizacje tablic), przygotowanie do
             szablonow 3x3 z dodatkowymi warunkami (Pattern3extra, Pattern3full) -- to nietestowane.
 wersja 131: Poprawione findInterior(), przez to, ze zle dzialalo, czasami sie wieszaly turnieje. Nie mialo wplywu na sama gre, tylko na odczytywanie sgf-ow.
 wersja 132: Dalszy rozwoj Pattern3extra; znaleziony blad w Game::Game(): nie bylo inicjowane pattern3_at[] na brzegu planszy, a tylko w naroznikach.
 wersja 133: Kilka nowych szablonow w Pattern3extra, w zasadzie kod przygotowany do dodania do generateListOfMoves() lub choose...Move().
 wersja 134: Pierwsza wersja wykorzystujaca Pattern3extra, na razie tylko w generateListOfMoves().
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v134, 3 rdzenie i7)
   Gracz      meczów  punkty     w    r     p   zdobyte:stracone  czas gr:przec   średnio    min--max
  1. MC500     274   241,5    241    1    32      3494:2540        2578s: 1013s   9.40s   8.36s--11.83s
  2. 4/0T      274    32,5     32    1   241      2540:3494        1013s: 2578s   3.69s   1.57s--10.38s
 88.1%, duzo lepiej (chyba, bo gier nie jest za duzo)

 6 vs MC 2000, plansza 12x12  (plik 6_vs_MC2000_v134, Athlon x3)
   Gracz      meczów  punkty     w    r     p   zdobyte:stracone  czas gr:przec   średnio    min--max
   1. MC2000   264   251,5    250    3    11      2367:1276        4912s:18198s  18.60s  14.57s--27.52s
   2. 6/0T     264    12,5     11    3   250      1276:2367       18198s: 4912s  68.93s  18.06s--187.6s
 95.3%, nieco gorzej, ale raczej za malo gier

 wersja 135: ???
 wersja 136: nowe szablony 'extra', rowniez zastepujace Pattern52
   Gracz  meczów  punkty     w    r     p   zdobyte:stracone  czas gr:przec   średnio    min--max
1. MC 500  1172  1021,5   1016   11   145     14485:10256      22307s: 8864s  19.03s  15.34s--26.90s
2. 4/0T    1172   150,5    145   11  1016     10256:14485       8864s:22307s   7.56s   2.30s--21.07s
 87.1%, lepiej niz v127, byc moze nieco gorzej niz v134 [rozgrywane na 3 z 2+2 rdzeni i3-4030U @ 1.90GHz]

   Gracz   meczów  punkty     w    r     p   zdobyte:stracone  czas gr:przec   średnio    min--max
1. MC 2000  4856  4718,5   4699   39   118     46060:21239      83754s: 4482m  17.24s  12.03s--23.45s
2. 6/0T     4856   137,5    118   39  4699     21239:46060       4482m:83754s  55.37s  12.72s--167.6s
 97.17%, prawdopodbnie lepiej niz najlepsza dotad v118 (96.1%); plansza: 12x12, 4 rdzenie i5-2400 @ 3.1 GHz

 wersja 137: zaczete bardziej skomplikowane threat2m, ale nie skonczone; rozgrzebany fragment zostal zakomentowany
 wersja 138: (po 7 miesiecznej przerwie): zmiany w Game::chooseAnyMove_pm(): nowa funkcja Game::getGoodTerrMoves(who)
             do dokladniejszego znajdowania sensownych ruchow do terytorium
             Poprawiony maly blad w findSimpleEnclosure(). Globalna zmienna empty_enclosure (odrobine przyspiesza).
 4 vs MC 500, plansza 14x14 (plik 4_vs_MC500_14x14_v138_athlon)
   Gracz                meczów  punkty     w    r     p   zdobyte:stracone  czas gr:przec   średnio    min--max
1. ./aopt138.out - 1000 500(B)  1956  1612,5   1587   51   318     22443:17854      30835s:13512s  15.76s  11.90s--35.87s
2. 4/0T             (A)  1956   343,5    318   51  1587     17854:22443      13512s:30835s   6.90s   2.30s--21.47s
   Łącznie:              1956  1956     1905  102  1905     40297:40297      44346.83s     22.67s
 82.4% (znacznie gorzej niz 134/136)

 6 vs MC 2000, plansza 12x12 (plik 6_vs_MC2000_v138_3, Athlon x3)
   Gracz                meczów  punkty     w    r     p   zdobyte:stracone  czas gr:przec   średnio    min--max
   1. ./aopt138.out - 1000 2000(B)   436   372,5    355   35    46      3490:2156       10046s:33579s  23.04s  16.34s--36.90s
   2. 6/0T             (A)   436    63,5     46   35   355      2156:3490       33579s:10046s  77.01s  27.63s--186.9s
   Łącznie:               436   436      401   70   401      5646:5646       43624.56s     100.05s
 85.4%, duzo GORZEJ! Wyglada, jakby byly usuwane dame, zob. np. mc138_wypelnia_od_107.sgf, w ktorym niebieski gra do terytorium i przegrywa.

 wersja 139: Poprawiony blad w checkThreat2moves_encl(), teraz sprawdzamy przed dodaniem zagrozenia checkIfRedundant(...).
             Blad zostal znaleziony dzieki temu, ze po dodaniu zagrozen ze zbednymi kropkami na brzegu (typu cc-cd-dd: cc zbedna),
             zle dzialaly shortcuts.
 wersja 140: Nowe dynamic komi, z green/red zone i ratchet.
             Testuje na 4 vs MC 500, plansza 14x14
                0.4 -- 0.85:  punkty: 99/130 = 74.4%, malo
                0.2 -- 0.85:  punkty: 135/157 = 86%, lepiej (byly bledy)
                bez bledow:
                0.45-- 0.65:  punkty: 300/353 = 85%, dobrze (plik  4_vs_MC500_v140_red045_green065)
                0.15-- 0.7 :  punkty: 151/174 = 86.8%, najlepiej/obiecujaco (4_vs_MC500_v140_red015_green070)
		0.1 -- 0.85:  punkty: 121.5/146=83.2%, gorzej  (plik 4_vs_MC500_v140_red010_green085)
		0.25-- 0.85:  punkty: 360/440 = 81.8%, gorzej (plik 4_vs_MC500_v140_red025_green085)
		0.2 -- 0.8 :  punkty: 328.5/397=82.7%, trudno powiedziec, bo za malo gier (plik 4_vs_MC500_v140_red020_green080)
                0.15-- 0.75:  punkty: 816.5/975=83.7%, niezle (plik 4_vs_MC500_v140_red015_green075)

	     Dla 4 vs MC2000, 14x14:
                0.2 -- 0.85:  punkty: 461/474 = 97.25%, super, ale to pewnie za duza przewaga MC (plik 4_vs_MC2000_v140_red020_green085_nobug)

             Dla 6 vs MC2000, plansza 12x12:
                0.2 -- 0.85:  punkty: 202/220 = 91.8%, lepiej niz ostatnio, ale bywalo jeszcze lepiej (6_vs_MC2000_v140_red020_green085)
  Przyjmuje 0.15/0.75. Bardziej ostroznie niz testy z MC500, bo MC500 robi tylko jedna zmiane komi, co moze oznaczac, ze czestsze zmiany sa lepsze.

./agdb140.out mc140_ruch69_czemu_nie_zamyka.sgf 68 300  
 --- bez sensu nie zamyka, bo ma za mala premie za zamykane kropki (za ratowane kropki pewnie ok)

 wersja 141: W Game::generateListOfMoves() zwiekszam premie za zamkniete kropki -- do tej pory premia byla taka sama za zamkniete, jak i za uratowane
             kropki, co jest bez sensu, bo np. ratowanie moze byc zle ze wzgledu na drabinke, podczas gdy zamkniecie wydaje sie uniwersalnie mocne.
 6 vs MC2000, plansza 12x12 (plik 6_vs_MC2000_v141)
   Gracz                meczów  punkty     w    r     p   zdobyte:stracone  czas gr:przec   średnio    min--max
   1. ./aopt141.out - 1000 2000(B)   704   624,5    596   57    51      6372:3639       22076s:63653s  31.35s  17.82s--63.81s
   2. 6/0T             (A)   704    79,5     51   57   596      3639:6372       63653s:22076s  90.41s  28.08s--257.8s
   Łącznie:               704   704      647  114   647     10011:10011      85729.04s     121.77s
 88.7%, mala poprawa w stosunku do 138
...
/********************************************************************************************************
  Enclosure class
*********************************************************************************************************
  There two lists: interior, border with the list of interior, border points of the enclosure.
...
{
  assert(seq.size() == debug_size);
  std::cout << "seq.size: " << seq.size() << std::endl;
  assert(Pattern3extra::MASK_DOT == MASK_DOT);
  komi = 0;
  komi_ratchet = 10000;  
  dame_moves_so_far = 0;
  auto itt = seq[0].findProp("SZ");
  std::string sz = (itt != seq[0].props.end()) ? seq[0].findProp("SZ")->second[0] : "";
	...
	for (auto &p : encl->border) {
	  if (stack[p]) {
	    ...
	    if (check_encl && (thr->type & Threat::ENCL)) {
	      #ifndef NDEBUG
	      std::cerr << "Sprawdzam zagr " << coord.showPt(thr->where) << std::endl;
	      #endif
...
Game::makeSgfMove(std::string m, int who)
{
  ...
  std::cerr << "[" << ++sgf_move_no << "] Make move " << who << " : " << m << " = " << coord.showPt(coord.ind(coord.sgfToX(m), coord.sgfToY(m))) << std::endl;
  
#endif
  // TODO: in must-surround, this must be done simultanously
  Move move;
  move.ind = coord.ind(coord.sgfToX(m), coord.sgfToY(m));
  move.who = who;
  /*
  placeDot(coord.sgfToX(m), coord.sgfToY(m), who);
  ...
  */
  ...
  int pos = 3;
  while (pos<=m.length()) {
    if (pos<m.length() && m[pos] != '.') {
      border.push_back( coord.ind( coord.sgfCoordToInt(m[pos]), coord.sgfCoordToInt(m.at(pos+1)) ));
      pos+=2;
    } else {
      // (... remove last element of border?)
      //std::cerr << "...enclosure: ";      for (pti p : border) std::cerr << coord.showPt(p) << " ";      std::cerr << std::endl;
      move.enclosures.push_back(std::make_shared<Enclosure>(findInterior(border)));
      /*
      auto encl = findInterior(border);
...
#endif
    Game tmp = pos;
    if ((i & 0x7f) == 0) std::cerr << "iteracja = " << i << std::endl;
    if (i >= komi_change_at) {
      komi_change_at *= 4;
...
MonteCarlo::runSimulations(Game pos, int max_iter_count, int thread_no)
{
  ...
  for (;;) {
    Game tmp = pos;
    if ((i & 0x7f) == 0) std::cerr << "thr " << thread_no << ", iteracja = " << i << std::endl;
    if (thread_no == 0) {
...
int main(int argc, char* argv[]) {
  ...
  auto f = seq[0].findProp("SZ");
  debug_size = seq.size();
  Game game(seq, (argc > 2 ? std::atoi(argv[2]) : 2000));
  auto end_time = std::chrono::high_resolution_clock::now();
  std::cerr << "init time: " << std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count() << " micros" << std::endl;
  //  std::cerr << "Enclosures: obowiazkowe: " << encl_count << ", opcjonalne: " << opt_encl_count << ", laczna wlk priority: " << priority_count << ", liczba ruchow: " << moves_count << std::endl;
#ifndef NDEBUG
  game.show();
#endif

  int iter_count = (argc > 3) ? std::atoi(argv[3]) : 50;
