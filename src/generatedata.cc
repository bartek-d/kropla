...
public:
  ...
  void storeStatsForPattern(pattern3_t p, bool wasMoveHere, int whoPlays, pti place, pti move0, pti move1);
  void show() const;
...
void
PatternStats::storeStatsForPattern(pattern3_t p, bool wasMoveHere, int whoPlays, pti place, pti move0, pti move1)
{
  if (whoPlays == 2) {
  ...
  if (coord.isInNeighbourhood(place, move0)) {
    if (present_once_close_to_move0.find(smallest) == present_once_close_to_move0.end()) {
  ...
  if (coord.isInNeighbourhood(place, move1)) {
    if (present_once_close_to_move1.find(smallest) == present_once_close_to_move1.end()) {
...
PatternStats::show() const
{
  ...
  while (not queue.empty()) {
    ...
    std::cout << "Frequency " << sp.stats.getFrequency() << " ("
	      ...
	      << " counted once: " << sp.stats.played  << " / " << sp.stats.present_once << " = " << static_cast<double>(sp.stats.played) / sp.stats.present_once
	      << " counted once close to m0: " << sp.stats.played_close_to_move0  << " / " << sp.stats.present_once_close_to_move0
	      << " = " << static_cast<double>(sp.stats.played_close_to_move0) / sp.stats.present_once_close_to_move0
	      << " counted once close to m1: " << sp.stats.played_close_to_move1  << " / " << sp.stats.present_once_close_to_move1
	      << " = " << static_cast<double>(sp.stats.played_close_to_move1) / sp.stats.present_once_close_to_move1
	      << ") --> pattern:\n"
...
pattern3_val
PatternStats::getValue(pattern3_t p_raw)
{
  ...
  if (most_frequent < 1.0) {
    most_frequent = std::accumulate(stats.begin(),
				    stats.end(), 0,
				    [](uint32_t maks, const auto& el) { return std::max(maks, el.present); });
  }
  double coeff = std::min(20 * s.present / most_frequent, 1.0);
  */
  double freq =  0.3* static_cast<double>(s.played) / (s.present_once + 9) + 0.7*static_cast<double>(s.played_close_to_move0) / (s.present_once_close_to_move0 + 9);
...
std::string PatternStats::getDescription(pattern3_t p_raw) const
{
  ...
  buffer << "Frequency " << sp.getFrequency() << " ("
	 << sp.played << " / " << sp.present
	 << " counted once: " << sp.played  << " / " << sp.present_once << " = " << static_cast<double>(sp.played) / sp.present_once
	 << " counted once close to m0: " << sp.played_close_to_move0  << " / " << sp.present_once_close_to_move0
	 << " = " << static_cast<double>(sp.played_close_to_move0) / sp.present_once_close_to_move0
	 << " counted once close to m1: " << sp.played_close_to_move1  << " / " << sp.present_once_close_to_move1
	 << " = " << static_cast<double>(sp.played_close_to_move1) / sp.present_once_close_to_move1
	 << ")";
  return buffer.str();
}
  ...
  for (int i=coord.first; i<=coord.last; ++i) {
    if (game.whoseDotMarginAt(i) == 0) {
      patt_stats.storeStatsForPattern(game.readPattern3_at(i), i == move.ind, move.who,
				      move.ind, game.getLastMove().ind, game.getLastButOneMove().ind);
    }
  }
}
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Number not present: total = " << not_present << ", without atari and not dame: " << no_atari_and_not_dame << ", dame: " << dame << std::endl;
  std::cout << "--------------------------------------------------" << std::endl;
  ...
  for (uint32_t p=0; p<PATTERN3_SIZE; ++p) {
    ...
    if (p3v < 0) {
      // hane: Frequency 0.0467318 (15874 / 339683 counted once: 15874 / 139751 = 0.113588 counted once close to m0: 9685 / 65636 = 0.147556 counted once close to m1: 7190 / 51300 = 0.140156) --> pattern:
	...
	if (ratio < 0.2 or ratio > 5.0) {
	  std::cout << "p3val = " << p3v << "  but stats_val = " << stats_val
