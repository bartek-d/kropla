...
bool Enclosure::isInInterior(pti p) const
{
...
bool Enclosure::isInBorder(pti p) const
{
    return (std::find(border.begin(), border.end(), p) != border.end());
}

bool Enclosure::isEmpty() const { return border.empty(); }
...
int Enclosure::getInteriorSize() const { return interior.size(); }

uint64_t Enclosure::zobristKey(int who) const
{
    uint64_t key = 0;
    who--;
...
/// Given a point p1 from the border and a point x!=p1 where the player just
/// played, it finds previous (p0) and next (p2) point from the border and
/// checks, whether x touches p0, p1 and p2. In particular, it may happen that
/// p1==x.
//  If yes, there is a shortcut.
/// @param[in] p1  Point from the border.
/// @param[in] x   Point where the player just played.
/// @return    bool value
bool Enclosure::checkShortcut(pti p1, pti x) const
{
    // if (p1==x) return true;
    if (!coord.isInNeighbourhood(p1, x)) return false;
    assert(border[0] == border[border.size() - 1]);
    unsigned nr = 0;
    while (nr < border.size() and border[nr] != p1) ++nr;
    if (nr > 0)
    {
        assert(nr + 1 < border.size());  // this should hold because border[0]
                                         // == border[border.size()-1]
        return (coord.isInNeighbourhood(x, border[nr - 1]) &&
                coord.isInNeighbourhood(x, border[nr + 1]));
    }
    else
    {
        return (coord.isInNeighbourhood(x, border[1]) &&
                coord.isInNeighbourhood(x, border[border.size() - 2]));
    }
}
...
/// @param[in] p1  Point from the border.
/// @return    bool value
bool Enclosure::checkIfRedundant(pti p1) const
{
    assert(border[0] == border[border.size() - 1]);
    unsigned nr = 0;
    while (nr < border.size() and border[nr] != p1) nr++;
...
std::string Enclosure::toSgfString() const
{
    std::string es;
    assert(!border.empty());
    for (auto &p : border) es += coord.indToSgf(p);
    if (border[0] < border[1])
        return "." + es;
    else
        return "." + es.substr(2) +
               es.substr(2, 2);  // could be always "."+es, but we want to save
                                 // in a zagram.org's style
}

SgfProperty Move::toSgfString() const
{
...
{
    return (who != -1)
               ? coord.showPt(ind) + " +" + std::to_string(enclosures.size()) +
                     " encl(s)" /*, zobr=" + std::to_string(zobrist_key)*/
               : "(none)";
}
