...
*********************************************************************************************************/
const constexpr real_t MC_SIMS_EQUIV_RECIPR = 1.0 / 100.0;  // originally 2500!
const constexpr real_t MC_SIMS_ENCL_EQUIV_RECIPR = 1.0 / 10.0;
...
real_t Treenode::getValue() const
{
    ...
    if (true)  // parent != this)    // not at root
    {
        ...
        const real_t C = is_root ? 1.4 : 0.14;
...
// second gives a code which neighbours may go inside.
{
    ...
    {
        auto border_group =
            descr.at(worm[smallest_terr->encl->getBorderElement()]).group_id;
        int count = 0;
        ...
        if (count >= 2)
        {
            ...
            else
            {
                ...
                /* debug:
                   ...
                   nowMoves << "] at " << coord.showPt(history.back() &
        ...
        {  // and dots>1)  we could check if there's more than 1 our dot in the
           // neighbourhood, as only then we need to check
            ...
            for (int j = 0; j < 8; j++)
            {
                ...
                if (isDotAt(nb) and descr.at(worm[nb]).group_id == border_group)
                    gdots++;
            }
            if (gdots > 1)
                smallest_terr->type |=
    ...
    if (top >= 2)
    {
        ...
        if (ug >= 2)
        {
            for (int i = coord.first; i <= coord.last; i++)
            {
                if (connects[who - 1][i].groups_id[1] !=
                            ...
                            i, ind))  // and does not touch [ind]
                {
                    ...
                    {
                        for (int j = 0; j < 4; j++)
                            if (connects[who - 1][i].groups_id[j] ==
                                unique_groups[k])
                            {
                                count++;
                                break;
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (connects[who - 1][nb].groups_id[0] != 0)
                ...
                if ((nb2 == ind) ||
                    (whoseDotMarginAt(nb2) == who and
                     std::find(groups.begin(), groups.end(),
                               descr.at(worm[nb2]).group_id) != groups.end()))
                {
                    w |= 1;
                }
                else if (whoseDotMarginAt(nb2) == 3)
...
bool Game::haveConnection(pti p1, pti p2, int who) const
{
            ...
            for (int j = p2[0]; j < p2[1]; j++)
            {
                ...
                for (int k = p3[0]; k < p3[1]; k++)
                {
                    ...
                    if (whoseDotMarginAt(nb3) == who and
                        connected_groups.contains(descr.at(worm[nb3]).group_id))
                    {
                        /*
                        // dla pokazania ustaw kropki na ind, nb, nb2
                        CleanupOneVar<pti> worm_where_cleanup0(&worm[ind],
                        who+4); CleanupOneVar<pti> worm_where_cleanup(&worm[nb],
                        who); CleanupOneVar<pti> worm_where_cleanup2(&worm[nb2],
                        ...
                        */
                        ...
                        who+4); CleanupOneVar<pti> worm_where_cleanup(&worm[nb],
                        who); CleanupOneVar<pti> worm_where_cleanup2(&worm[nb2],
                        who); show(); std::cerr << "Nieoczekiwanie dodaje (ind="
                        << coord.showPt(ind) << "): " << coord.showPt(nb) << ",
                        " << coord.showPt(nb2) << " --> " << coord.showPt(nb3)
                        << std::endl; std::cin.ignore();
                        }
        ...
        connected_groups.hasAtLeastTwoDistinctElements())
    {
             ...
             ++first)
        {
                 ...
                 ++second)
            {
                ...
                {
                    if (not connects[who - 1][point_close_to_1].contains(
                            second_gr_id))
                        continue;
                 ...
                 second < unique_connected_groups.size(); ++second)
            {
                ...
                for (auto other_gr_id : group_neighb[first].neighbour_groups)
                {
                         ...
                         group_neighb[first].neighbours_list)
                    {
                        ...
                        {
                            if (not connects[who - 1][point_close_to_2]
    ...
    const SmallMultiset<pti, 4> &connected_groups, pti ind, int who) const
{
    ...
    for (int i = 0; i < 8; i++)
    {
        pti nb = ind + coord.nb8[i];
        if (whoseDotMarginAt(nb) != 0 ||
            connects[who - 1][nb].groups_id[0] == 0)
            continue;
        std::array<pti, 4> unique_groups = {0, 0, 0, 0};
...
/// @param[in] who  Whose threat it is (i.e., who makes this enclosure).
{
    ...
    if (worm[where] == 0)
    {
        CleanupOneVar<pti> worm_where_cleanup(
            &worm[where], who);  //  worm[where] = who;  with restored old value
                                 //  (0) by destructor
        int done[4] = {0, 0, 0, 0};
        for (int j = 0; j < 4; j++)
            if (!done[j])
...
void Game::checkThreats_postDot(std::vector<pti> &newthr, pti ind, int who)
{
    ...
    */
    ...
    while (!newthr.empty())
    {
        ...
        {
            CleanupOneVar<pti> worm_where_cleanup(
                &worm[where], who);  //  worm[where] = who;  with restored old
...
/// @param[in] who  Whose threat it is (i.e., who makes this enclosure).
{
    ...
    if (worm[where] == 0 and worm[where0] == 0)
    {
        CleanupOneVar<pti> worm_where_cleanup0(
            &worm[where0], who);  //  worm[where0] = who;  with restored old
                                  //  value (0) by destructor
        CleanupOneVar<pti> worm_where_cleanup(
            &worm[where], who);  //  worm[where] = who;  with restored old value
                                 //  (0) by destructor
                                      ...
                                      int who)
{
    ...
    while (!newthr.empty())
    {
        ...
        {
            CleanupOneVar<pti> worm_where_cleanup0(
                &worm[ind0], who);  //  worm[ind0] = who;  with restored old
                                    //  value (0) by destructor
            CleanupOneVar<pti> worm_where_cleanup1(
                &worm[ind1], who);  //  worm[ind1] = who;  with restored old
                                    //  value (0) by destructor
            debug_allt2m++;
            assert(!newthr.empty());
...
void Game::connectionsRecalculateCode(pti ind, int who)
{
...
void Game::connectionsRecalculateConnect(pti ind, int who)
{
    int g = who - 1;
...
void Game::connectionsRecalculatePoint(pti ind, int who)
{
...
void Game::connectionsRecalculateNeighb(pti ind, int who)
{
    for (int i = 0; i < 8; i++)
    {
        pti nb = ind + coord.nb8[i];
...
real_t Game::getPattern52Value(pti ind, int who) const
{
    ...
    for (int k = 0; k <= 1; k++)
    {
        ...
        if (k == 0)
        {
            if (coord.y[ind] > 0 and coord.y[ind] < coord.wlky - 1)
            {
                ...
                {
                    std::array<pti, 9> ds = {coord.NNE, coord.NN, coord.NE,
                                             coord.N,   coord.E,  coord.SSE,
                                             coord.SS,  coord.SE, coord.S};
                    deltas = ds;
                    type = (coord.x[ind] == 0) ? EDGE : INNER;
                }
                else if (coord.x[ind] == 1 || coord.x[ind] == coord.wlkx - 1)
                {
                    std::array<pti, 9> ds = {coord.NN,  coord.NNW, coord.N,
                                             coord.NW,  coord.W,   coord.SS,
                                             coord.SSW, coord.S,   coord.SW};
                    deltas = ds;
        ...
        {  // k==1
            if (coord.x[ind] > 0 and coord.x[ind] < coord.wlkx - 1)
            {
                ...
                {
                    std::array<pti, 9> ds = {coord.SWW, coord.WW, coord.SW,
                                             coord.W,   coord.S,  coord.SEE,
                                             coord.EE,  coord.SE, coord.E};
                    deltas = ds;
                    type = (coord.y[ind] == 0) ? EDGE : INNER;
                }
                else if (coord.y[ind] == 1 || coord.y[ind] == coord.wlky - 1)
                {
                    std::array<pti, 9> ds = {coord.EE,  coord.NEE, coord.E,
                                             coord.NE,  coord.N,   coord.WW,
                                             coord.NWW, coord.W,   coord.NW};
                    deltas = ds;
        ...
        if (type != NONE)
        {
            ...
            value += (type == EDGE) ? global::patt52_edge.getValue(p, who)
                                    : global::patt52_inner.getValue(p, who);
        }
    }
    return value;
}
                          ...
                          int escapes, int iteration)
{
    ...
    if ((worm[nx] & MASK_DOT) == 0 || ladder_ext || curr_ext)
    {
        ...
        // ladder
        if (((worm[nx + v1] | worm[nx + v2]) & MASK_DOT) == attacks and
            !curr_ext)
        {
            return ATT_WINS;
        }
        ...
        for (pti w : {(pti)(-v2), v2})
        {
            if ((worm[nx + v1 - w] & MASK_DOT) == 0 and
        ...
        if (iteration >= 2)
        {
            ...
            if (((worm[nx + v1 - v2] | worm[nx - 2 * v2]) & MASK_DOT) ==
                escapes)
            {
                return ESC_WINS;
            }
        ...
        if ((worm[nx + v1] & MASK_DOT) == escapes)
        {
            ...
            ladder_ext = ((worm[nx + v1] & MASK_DOT) != 0);
...
void Game::show() const
{
    ...
    std::cerr << "Score: " << score[0].show() << "; " << score[1].show()
              << std::endl;
}
...
std::vector<pti> Game::getPatt3extraValues() const
{
    ...
    for (int i = coord.first; i <= coord.last; ++i)
    {
        ...
        {
            global::patt3_extra.setValues(values, worm, pattern3_at[i], i,
                                          nowMoves);
...
std::vector<pti> Game::findImportantMoves(pti who)
{
    ...
    {
        if (whoseDotMarginAt(p) == 0 and threats[0].is_in_terr[p] == 0 and
            threats[0].is_in_encl[p] == 0 and threats[1].is_in_terr[p] == 0 and
            threats[1].is_in_encl[p] == 0)
        {
            float opp_value, our_value, opp_value2;
            CleanupOneVar<pti> worm_where_cleanup0(
                &worm[p], who);  //  worm[where0] = who;  with restored old
                                 //  value (0) by destructor
            CleanupUsingListOfValues<decltype(pattern3_at) &, pattern3_t>
                pattern3_cleanup(pattern3_at);
            {
    ...
    if (ten == 0) ten = 1;
    float median = (moves.size() & 1) ? moves[moves.size() / 2].value
                                      : (moves[moves.size() / 2 - 1].value +
                                         moves[moves.size() / 2].value) *
                                            0.5;
    ...
    show();
    std::cerr << "Ciekawe ruchy dla gracza " << who << ": " << ten*2 <<
    std::endl; std::cerr << coord.showBoard(list);
    */
    return list;
}
...
float Game::floodFillCost(int who) const
{
    struct PointCost
    {
        pti point;
        ...
        }  // warning: > to make 'cheaper' points of higher priority!
    };
    struct PointCostHigh : PointCost
    {
        PointCostHigh(pti p, float c)
            : PointCost(p, c){};  // { point=p; cost=c; };
        bool operator<(const PointCost &other) const
        {
            return cost < other.cost;
        }
    ...
    // top and bottom edge
    {
        ...
        for (int i = 2; i < coord.wlkx; i++)
        {  // 2,...,coord.wlkx-1, to omit the corners
    ...
    while (!queue.empty())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (dad[nb] == 0 and coord.dist[nb] >= 1)
            {  // edge has been already visited, therefore >=1, not 0
    ...
    */
    ...
    for (int p = coord.first; p <= coord.last; p++)
    {
        if ((whoseDotMarginAt(p) == 0 || whoseDotMarginAt(p) == (who ^ 3)) and
            coord.dist[p] >= 1)
        {
            if (isInTerr(p, who) > 0 || isInEncl(p, who) > 0)
            {
                if (whoseDotMarginAt(p) == 0)
                    value += 0.5;
                else               // dot of 3-who
                    value += 1.0;  // this is simplified, it may be an empty
                                   // point inside who's enclosure, which is not
                                   // worth 1 point
            }
            else if (costs[p] >= 0.1)
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    if (whoseDotMarginAt(nb) == who || costs[nb] >= 0.01 ||
                        isInTerr(nb, who) > 0 || isInEncl(nb, who) > 0)
                    {
                        // ok
                    }
                    else
    ...
    std::cerr << "Plansza: " << std::endl;
    show();
    std::cerr << "Ciekawe ruchy dla gracza " << who << ": " <<
    interesting_moves.size() << std::endl; std::cerr <<
    coord.showBoard(debug_board); for (int i=coord.first; i<=coord.last; i++) {
    ...
    std::cerr << "costs: " << std::endl << coord.showBoard(debug_2) <<
    std::endl; std::cerr << "tree costs: " << std::endl <<
    coord.showBoard(debug_tc) << std::endl; std::cerr << "dad: " << std::endl <<
    coord.showBoard(dad) << std::endl;
    */
}
...
void Game::wormMergeAny(pti dst, pti src)
...
void Game::wormMergeOther(pti dst, pti src)
...
void Game::wormMergeSame(pti dst, pti src)
{
...
void Game::wormMerge_common(pti dst, pti src)
...
//   == false:  if 'encl' is in threats, it has to be found.
{
    ...
    auto updateSingInfo = [&](pti leftmost)
    {
        if (isInEncl(leftmost, 3 - who) + isInTerr(leftmost, 3 - who) == 1)
        {
            ...
            singular_worms.insert(
                {leftmost, descr.at(worm[leftmost]).dots[who - 1]});
        }
        else
        {
    ...
    for (auto &p : encl.border)
    {
        if (worm[p] != worm_no)
        {
            ...
            if ((descr.at(worm[p]).safety <= 1 ||
                 descr.at(worm_no).safety <= 1) and
                (descr.at(worm[p]).safety + descr.at(worm_no).safety >= 2))
            {
                update_safety_dame = true;
            }
            if ((descr.at(worm[p]).safety <= 1 ||
                 descr.at(worm_no).safety <= 1) and
                (descr.at(worm[p]).safety + descr.at(worm_no).safety >= 1))
            {
                update_soft_safety = safety_soft.getUpdateValueForAllMargins();
            }
    ...
    if (!gids_to_delete.empty())
    {
        for (int ind = coord.first; ind <= coord.last; ind++)
        {
            for (int j = 0; j < 4; j++)
            {
                ...
                if (std::find(gids_to_delete.begin(), gids_to_delete.end(),
                              connects[2 - who][ind].groups_id[j]) !=
                    gids_to_delete.end())
                {
    ...
    */
    ...
    for (auto &e : poolsB)
    {
        ...
        if (ok)
        {
            ...
            {
                if ((worm[ind] & MASK_DOT) == 2 and
                    (marks[descr.at(worm[ind]).leftmost] & ct_COUNTED) == 0)
                {
                    // take it
                ...
                else if ((worm[ind] & MASK_DOT) == 0 and
...
std::pair<int, int> Game::countTerritory_simple(int now_moves) const
{
    ...
    if ((score[0].dots - score[1].dots) + delta != ct_score.first or
        small_score != ct_score.second)
    {
        show();
        std::cerr << "res = " << (score[0].dots - score[1].dots) + delta
    ...
    for (pti p : encl.interior)
    {
        if (whoseDotMarginAt(p) == 0)
            ++terr;
                 ...
                 counted.find(worm[p]) == counted.end())
        {
...
void Game::makeSgfMove(const std::string &m, int who)
{
    ...
    if (m == "ek") {
      ...
      for (auto &t : threats[1].threats2m) {
        ...
        std::cout << "  min_wins: " << t.min_win << ", " << t.min_win2 <<
    std::endl; std::cout << "  flags: " << t.flags << ", win_move_count: " <<
    t.win_move_count << std::endl; if (not t.is_in_encl2.empty()) { std::string
    filename{"is_in_encl2__" + std::to_string(i) + ".svg"};
          ++i;
        ...
        for (const auto& thr : t.thr_list) {
          std::cout << "   ... " << coord.showPt(thr.where) << " opp_dots: " <<
    thr.opp_dots << std::endl;
        }
      }
    }
    */
    ...
    //#ifndef NDEBUG
    {
        ...
        // auto it = history.end();
        //--it;
        // Move m = choosePattern3Move((*it) & history_move_MASK, (*(it-1)) &
        // history_move_MASK, 3-who); Move m = chooseAnyMove(3-who);
#ifndef NDEBUG
        /*
        if (m.ind) {
        ...
        threats[1].is_in_encl[p] == 0 and threats[1].is_in_terr[p] == 0)
    {
        ...
        if (coord.dist[p] == 0)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (coord.dist[nb] == 1)
                {
                    if (worm[nb] and descr.at(worm[nb]).isSafe())
                        return PossibleMovesConsts::DAME;
                    break;
                }
            }
...
void Game::possibleMoves_updateSafetyDame()
{
    if (!possible_moves.left)
    {
        ...
        for (int y = coord.wlky - 2; y > 0; --y)
        {
            ...
            {
                possible_moves.changeMove(p,
                                          descr.at(worm[p + coord.E]).isSafe()
                                              ? PossibleMovesConsts::DAME
                                              : PossibleMovesConsts::NEUTRAL);
            }
            p += coord.S;
    ...
    if (!possible_moves.right)
    {
        ...
        for (int y = coord.wlky - 2; y > 0; --y)
        {
            ...
            {
                possible_moves.changeMove(p,
                                          descr.at(worm[p + coord.W]).isSafe()
                                              ? PossibleMovesConsts::DAME
                                              : PossibleMovesConsts::NEUTRAL);
            }
            p += coord.S;
    ...
    if (!possible_moves.top)
    {
        ...
        for (int x = coord.wlkx - 2; x > 0; --x)
        {
            ...
            {
                possible_moves.changeMove(p,
                                          descr.at(worm[p + coord.S]).isSafe()
                                              ? PossibleMovesConsts::DAME
                                              : PossibleMovesConsts::NEUTRAL);
            }
            p += coord.E;
    ...
    if (!possible_moves.bottom)
    {
        ...
        for (int x = coord.wlkx - 2; x > 0; --x)
        {
            ...
            {
                possible_moves.changeMove(p,
                                          descr.at(worm[p + coord.N]).isSafe()
                                              ? PossibleMovesConsts::DAME
                                              : PossibleMovesConsts::NEUTRAL);
            }
            p += coord.E;
...
bool Game::checkWormCorrectness() const
{
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    if ((worm[nb] & MASK_DOT) == who)
                    {
                        ...
                        {  // diagonal connection
                            ...
                            if (std::find(test_descr[worm[nb]].neighb.begin(),
    ...
    for (int x = 0; x < coord.wlkx; x++)
    {
        ...
        if (worm[coord.ind(x, coord.wlky - 1)])
            test_descr.at(worm[coord.ind(x, coord.wlky - 1)]).safety =
        ...
        if (x > 0 and x < coord.wlkx - 1)
        {
            ...
            if (worm[coord.ind(x, coord.wlky - 1)] == 0 and
                worm[coord.ind(x, coord.wlky - 2)])
    ...
    for (int y = 0; y < coord.wlky; y++)
    {
        ...
        if (worm[coord.ind(coord.wlkx - 1, y)])
            test_descr.at(worm[coord.ind(coord.wlkx - 1, y)]).safety =
                WormDescr::SAFE_VALUE;
        if (y > 0 and y < coord.wlky - 1)
        {
            if (worm[coord.ind(0, y)] == 0 and worm[coord.ind(1, y)])
                test_descr.at(worm[coord.ind(1, y)]).safety++;
            if (worm[coord.ind(coord.wlkx - 1, y)] == 0 and
                worm[coord.ind(coord.wlkx - 2, y)])
    ...
    for (int ind = coord.first; ind <= coord.last; ind++)
    {
            ...
            s.isDameFor(2, ind) != safety_soft.isDameFor(2, ind))
        {
            ...
            std::cerr << "bledne is dame for dla " << coord.showPt(ind)
                      << " jest: " << safety_soft.isDameFor(1, ind) << " "
                      << safety_soft.isDameFor(2, ind)
            ...
            {
                {
                    ...
                    t.encl = std::make_shared<Enclosure>(
                        findEnclosure_notOptimised(ind, MASK_DOT, who));
                    if (!t.encl->isEmpty())
                    {
                        t.type = ThreatConsts::TERR;
                        t.where = 0;
        ...
        for (auto &thr2 : threats[who - 1].threats2m)
        {
            ...
            for (auto &thr : thr2.thr_list)
            {
                ...
                if (worm[where] == 0 and worm[where0] == 0)
                {
                    CleanupOneVar<pti> worm_where_cleanup0(
                        &worm[where0],
                        who);  //  worm[where0] = who;  with restored old value
                               //  (0) by destructor
                    CleanupOneVar<pti> worm_where_cleanup(
                        &worm[where],
                        who);  //  worm[where] = who;  with restored old value
                               //  (0) by destructor
                    int done[4] = {0, 0, 0, 0};
                    for (int j = 0; j < 4; j++)
                        if (!done[j])
                        {
                            pti nb = where + coord.nb4[j];
                            if ((worm[nb] & MASK_DOT) != who and
                                coord.dist[nb] >= 1 and
                                thr.encl->isInInterior(nb))
                            {
                                Threat t;
                                t.encl = std::make_shared<Enclosure>(
                                    findEnclosure_notOptimised(nb, MASK_DOT,
                                                               who));
...
bool Game::checkConnectionsCorrectness()
{
    ...
    for (int ind = 0; ind < coord.getSize(); ind++)
    {
        if (connects[0][ind] != tmp[0][ind] || connects[1][ind] != tmp[1][ind])
        {
            ...
            {
                for (int j = 0; j < 4; j++)
                    std::cerr << int(connects[g][ind].groups_id[j]) << " ";
                if (g == 0)
                    std::cerr << ",";
                else
                    std::cerr << std::endl;
