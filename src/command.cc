...
int CommandParser::parseCommandName()
{
    auto saved = pos;
    eatWS();
    std::pair<std::string, int> commands[] = {
        {"threads", 1}, {"iters", 1}, {"first", 1}, {"second", 1},
...
// Parses coord, throws an exception if no coord to parse or the coord is
// invalid.
void CommandParser::parseCoord()
{
    std::string point;
    for (int c = 0; c < 2; ++c)
    {
...
std::string CommandParser::spaces(int n) const
{
    std::string s;
    for (; n > 0; --n) s += " ";
    return s;
...
std::vector<std::string> CommandParser::parse(std::string b)
{
    buf = b;
    parsed.clear();
    pos = 0;
