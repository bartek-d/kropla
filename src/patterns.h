...
/* Each pattern is encoded in 20 bits.
   There is 2 bits for each of the 8 neighbours plus 4 for direct neighbours to
   save ataris: 6 7 0   atari:   b 5 . 1          a   8 4 3 2            9 Bits
   for neighbours: 0=empty, 1,2=dot, 3=outside, bits in atari may be set ONLY if
   there's a dot at a point, and set bit denotes dot in atari.
*/
...
class Pattern3
{
   ...
   public:
    ...
    void showCode() const;
    //  static const constexpr T RECALCULATE = 10000.0;  // this constant should
    //  be greater than any possible pattern value
    static bool isPatternPossible(pattern3_t p) { return p < PATTERN3_SIZE; };
    //  static const constexpr T RECALCULATE_TO_0 = 10001.0;  // this constant
