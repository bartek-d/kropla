...
struct Threat
{
    pti where;  // where to put dot, or 0 if no need to put a dot (so a
                // TERRitory)
    uint16_t type{0};
    int16_t terr_points{0};  // == number of empty places inside
    int16_t opp_dots{0};
    int16_t singular_dots{
    ...
    // SmallMultiset<pti, 4> shortcuts;  // this is only used for Threat2m
    // (TODO: maybe inherit a new class with this field?)
    // Threat() : encl(nullptr) { }
    Threat() : shortcuts() {}
...
constexpr uint16_t FLAG_RECALCULATE =
    2;  // should we recalcuate min_wins and miai flag after removal
constexpr uint16_t ENCL2_MIAI =
    1;  // this bit we set in is_in_encl2 to denote miai (i.e., exists other
        // threat with opp-dots win)
constexpr uint16_t ENCL2_INSIDE_ADD =
    2;  // this number we add for each point inside threat
constexpr uint16_t ENCL2_INSIDE_THRESHOLD =
...
struct Threat2m
{
    ...
    int16_t min_win{0}, min_win2{0};  // minimal win, second minimal win (i.e.,
                                      // how many opponent dots are captured)
    uint16_t flags{
        0};  // currently only FLAG_SAFE, in which case the threats get counted
...
struct AllThreats
{
    ...
    AllThreats()
        : is_in_encl(coord.getSize(), 0),
          ...
          is_in_border(coord.getSize(), 0),
          is_in_2m_encl(coord.getSize(), 0),
          is_in_2m_miai(coord.getSize(), 0){};
    // AllThreats(const AllThreats& other);
    int getMinAreaOfThreatEnclosingPoint(pti ind) const;
    int addThreat2moves(pti ind0, pti ind1, bool safe0, bool safe1, int who,
                        Threat &t);
