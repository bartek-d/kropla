...
/*
 Produces an array temp[j] of ints with the following entries:
   j encodes the position, each 2 bits: 0=empty, 1,2=players dot, 3=outside; the
 order of bits: NE, E, SE, S, SW, W, NW, N. Then temp[j] == 0, if the centre
 point does not connect 2 groups of player 1, otherwise, temp[j] encodes numbers
 of points (0-7) in four subsequent 3-bit-groups (always the last, so the next
 point does not belong to player 1 and might be enclosed); so it uses 12 bits.
 Bit 13 says if this is an edge pattern.
 If there are less then 4 groups connected, then the last number is repeated.
 If the pattern touches the edge, then only the point after the first might be
 enclosed.

 And also array simplified[j][4] with the entries similar as above, but:
  j encodes the position, each bit: 0=empty, 1=player dot;
 temp[j] == {n0, n1, n2, n3},  n--positions of four groups (or -1, if less than
 4 groups). Also possible that n0>=0 but n1==n2==n3==-1, unlike temp[j].
*/
...
class Coord
{
   public:
    ...
    static pti nb8[8];  // 8 neighbours, order is important for some functions
                        // (NE, E, ... -- clockwise)
    static pti first, last;  // smallest and largest coordinate possible for a
                             // point inside the board
    Coord(int x, int y);
...
Coord::Coord(int x, int y) { changeSize(x, y); }
...
//   6 7 8.
{
    ...
    if (otoczenie[1])
    {
        // if (otoczenie[0]) otoczenie[0]=1;  // -nie trzeba, bo to i tak =0 lub
        // 1
        otoczenie[1] = 1;
    ...
    // ustaw ew. narozniki
    if (otoczenie[0] && !otoczenie[1] && !otoczenie[3]) otoczenie[0] = ++ost;
    if (otoczenie[2] == 3) otoczenie[2] = ++ost;
    if (otoczenie[6] == 6) otoczenie[6] = ++ost;
    if (otoczenie[8] == 8) otoczenie[8] = ++ost;
    return ost;
}
    ...
    for (int p = 0; p < 256 * 256; p++)
    {
        ...
        if (banda >= 0)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                if (ot[kolejn[i]])
                {
                    nonzero = numery[kolejn[i]];
                    break;
                }
            }
            // popraw narozniki: jesli naroznik jest pusty, a pola obok niego
            // nie, to wypelniamy naroznik
            ...
            if (ile >= 2)
            {
                ...
                /*
                if (banda>=1)	{
                  std::cout << p << " " << std::hex << buf[p] << " banda=" <<
                banda << std::endl; pokazSzabl(p, buf[p]);
                  } */
                //
            }
            if ((p & simple_mask) == 0)
