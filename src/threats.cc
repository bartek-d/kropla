...
void Threat::addShortcuts(pti ind0, pti ind1)
{
    shortcuts.fill(-1);
    int count = 0;
    ...
    for (pti ind : {ind0, ind1})
    {
        ...
        {
            assert(nr + 1 <
                   encl->border.size());  // this should hold because border[0]
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (coord.isInNeighbourhood(nb, prev) and
                coord.isInNeighbourhood(nb, next))
            {  // and !shortcuts.contains(nb)) {
                // shortcuts.insert(nb);
                assert(count < 4);
                shortcuts[count++] = nb;
            }
...
std::string Threat::show() const
{
    std::stringstream out;
    out << "Typ = " << type << ", zobr = " << /*std::hex << */ zobrist_key
        << ", gdzie=" << coord.showPt(where) << std::endl;
...
void removeMarkedThreats(std::vector<Threat> &thr_list)
{
    ...
    for (int i = 0; i < s; ++i)
    {
        if (thr_list[i].type & ThreatConsts::TO_REMOVE)
        {
            ...
            {
                --s;
            } while (i < s and (thr_list[s].type & ThreatConsts::TO_REMOVE));
...
std::string Threat2m::show() const
{
    std::stringstream out;
    out << "Threat2m: where0 = " << coord.showPt(where0)
        << ", flags = " << std::hex << flags
        << ", win_move_count=" << win_move_count
        << ", threat count = " << thr_list.size() << std::endl;
    for (auto &t : thr_list)
    {
        out << t.show() << std::endl;
    }
...
void removeEmptyThreats2m(std::vector<Threat2m> &thr_list)
{
    ...
    for (int i = 0; i < s; ++i)
    {
        if (thr_list[i].thr_list.empty())
        {
            ...
            {
                --s;
            } while (i < s and thr_list[s].thr_list.empty());
            if (i < s) std::swap(thr_list[i], thr_list[s]);
        }
    }
    thr_list.erase(thr_list.begin() + s, thr_list.end());
...
AllThreats::AllThreats(const AllThreats& other)
{
  ...
  std::copy( other.threats.begin(), other.threats.end(),
std::back_inserter(threats) ); std::copy( other.threats2m.begin(),
other.threats2m.end(), std::back_inserter(threats2m) ); is_in_encl =
other.is_in_encl; is_in_terr = other.is_in_terr; is_in_border =
other.is_in_border; is_in_2m_encl = other.is_in_2m_encl; is_in_2m_miai =
other.is_in_2m_miai;
}
*/

void AllThreats::changeEnclToTerr(const Threat &t)
{
...
void AllThreats::addThreat2moves_toStats(Threat2m &t2, Threat &t)
{
...
void AllThreats::addThreat2moves_toMiai(Threat2m &t2, Threat &t)
{
    assert(!t2.is_in_encl2.empty());
    if (t2.win_move_count >= 2 || (t2.win_move_count == 1 and t.opp_dots == 0))
    {
...
/// @param[in] t  Threat after ind0-ind1 moves, with t.type, t.zobrist_key,
/// t.encl and t.opp_dots set.
                                ...
                                int /*who*/, Threat &t)
{
    ...
    {
        // game.show();
        std::cerr << "Redundant point0 in thr2m: " << coord.showPt(ind0)
                  << " second point: " << coord.showPt(ind1)
                  << " enclosure:" << std::endl
                  << t.encl->show() << std::endl;
        assert(0);
    }
    if (t.encl->checkIfRedundant(ind1))
    {
        // game.show();
        std::cerr << "Redundant point1 in thr2m: " << coord.showPt(ind1)
                  << " second point: " << coord.showPt(ind0)
                  << " enclosure:" << std::endl
                  << t.encl->show() << std::endl;
        assert(0);
    }
    ...
    for (int i = 0; i < 2; i++, std::swap(ind0, ind1), std::swap(safe0, safe1))
    {
        ...
        {
            // if (!pos->is_in_encl2.empty()) std::cerr << "is_in_encl2[" <<
            // coord.showPt(ind0) << "] przed:" <<
            // coord.showBoard(pos->is_in_encl2) << std::endl;
            auto pos2 = std::find_if(
                pos->thr_list.begin(), pos->thr_list.end(),
            ...
            if (pos2 != pos->thr_list.end())
            {
                ...
                if (pos2->type & ThreatConsts::TO_REMOVE)
                {
                    ...
                    continue;  // this threat has been found before;  used to be
                               // 'break' (v120-), we now (v121+) use continue
                               // to reset TO_REMOVE also in the second copy of
                               // this threat
                }
                else
                {
                    break;  // v121+: now we only break if bit TO_REMOVE was not
                            // set
                }
            }
            t.where = ind1;
                    ...
                    case 1:
                        if (pos->is_in_encl2.empty())
                        {
                            ...
                            for (auto &tt : pos->thr_list)
                                addThreat2moves_toStats(*pos, tt);
                        }
                        else
                        {
                            for (auto &tt : pos->thr_list)
            ...
            //
            // if (!pos->is_in_encl2.empty()) std::cerr << "is_in_encl2[" <<
            // coord.showPt(ind0) << "] po:" <<
            // coord.showBoard(pos->is_in_encl2)
            // << std::endl;
        }
    }
    return added;
...
void AllThreats::subtractThreat2moves(Threat2m &t2, const Threat &t)
{
    if (!t2.is_in_encl2.empty())
    {
        for (pti p : t.encl->interior)
        {
            ...
            t2.is_in_encl2[p] -= Threat2mconsts::ENCL2_INSIDE_ADD;
            if (was and
                t2.is_in_encl2[p] < Threat2mconsts::ENCL2_INSIDE_THRESHOLD and
...
void AllThreats::deleteMiai(Threat2m &t2)
{
...
void AllThreats::recalculateMiai(Threat2m &t2)
{
...
void AllThreats::changeFlagSafe(Threat2m &t2)
{
    t2.flags ^= Threat2mconsts::FLAG_SAFE;
...
Threat *AllThreats::findThreatWhichContains(pti ind)
{
    for (auto &t : threats)
    {
...
void AllThreats::turnOffThreats2m()
{
    if (not active_thr2m) return;
    threats2m.clear();
...
void AllThreats::removeMarkedAndAtPoint2moves(pti ind)
{
    if (!threats2m.empty())
    {
        ...
        for (auto &t2 : threats2m)
        {
            ...
            if (to_remove)
            {
                //	t2.thr_list.erase( std::remove_if( t2.thr_list.begin(),
                // t2.thr_list.end(), [](Threat &t) { return (t.type &
                // ThreatConsts::TO_REMOVE); } ), t2.thr_list.end() );
                ...
                {
                    to_remove2 = true;
                    if (t2.flags & Threat2mconsts::FLAG_RECALCULATE)
                        deleteMiai(t2);
                }
                else if (t2.flags & Threat2mconsts::FLAG_RECALCULATE)
                    recalculateMiai(t2);
            }
        }
        if (to_remove2)
...
void AllThreats::removeMarked2moves()
{
    if (!threats2m.empty())
    {
        bool to_remove2 = false;
...
int AllThreats::getMinAreaOfThreatEnclosingPoint(pti ind) const
{
...
std::string ThrInfo::show() const
{
    std::stringstream out;
    out << "lost_terr=" << lost_terr_points << ", won_dots=" << won_dots
        << ",  saved_dots=" << saved_dots
        << ", priority_value=" << priority_value;
    return out.str();
}

pti ThrInfo::calculatePriorityValue() const
{
    return -lost_terr_points + saved_dots * ThrInfoConsts::VALUE_SAVED_DOT +
           won_dots * ThrInfoConsts::VALUE_WON_DOT;
}
