...
class Coord
{
   public:
    ...
    pti nb8[32];   // 8 neighbours repeated 4 times, order is important for some
                   // functions (NE, E, ... -- clockwise), repeat is to avoid
                   // nb8[(i+...)&7]
    pti nb25[25];  // the central point and its 24 neighbours
    pti first, last;  // smallest and largest coordinate possible for a point
                      // inside the board
...
struct Score
{
    ...
    //  int terr;   // territory
    // int dots_in_terr;  // dots not enclosed yet, but captured inside
    // territory
