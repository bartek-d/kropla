...
std::string getPattStr(const pattern3_t p)
{
    ...
    for (auto c : code)
    {
        if (symbols.find(c) != symbols.end()) res += std::string{c};
    }
    const std::string out = res.substr(1, 2) + res.substr(5, 1) +
                            res.substr(8, 1) + res.substr(7, 1) +
                            res.substr(6, 1) + res.substr(3, 1) +
                            res.substr(0, 1);
...
{
    if (ps.at(4) == '#' and (ps.at(3) != '#' or ps.at(5) != '#')) return false;
    if (ps.at(4) != '#' and (ps.at(3) == '#' and ps.at(5) == '#')) return false;
    if (ps.at(4) == '#' and ps.at(0) == '#') return false;
    if (ps.at(4) == '#' and (ps.at(2) == '|' or ps.at(2) == '@' or
                             ps.at(6) == '|' or ps.at(6) == '@'))
        return false;
    if (ps.at(3) == '#' and (ps.at(2) != '#' or ps.at(1) != '#') and
        (ps.at(4) != '#' or ps.at(5) != '#'))
        return false;
    return true;
}
...
{
    if (ps.at(4) != 'Y' and ps.at(4) != '|') return true;
    for (int i = 5; i <= 10; ++i)
    {
        if (ps.at(i % 8) != 'Y' and ps.at(i % 8) != '|' and ps.at(i % 8) != '.')
            break;
        if (ps.at(i % 8) == '.') continue;
        if (i % 2 == 0 and ps.at(i % 8) != ps.at(4)) return false;
    }
    return true;
}
...
std::set<pattern3_t> generateAllPossibleSmallest()
{
    ...
    for (unsigned p = 0; p < PATTERN3_SIZE; ++p)
    {
        ...
        if (ps.find('E') != std::string::npos or
            ps.find('F') != std::string::npos)
            continue;
        rotations[0] = p;
        ...
        if (std::any_of(rotations.begin() + 1, rotations.end(),
                        [p](auto q) { return q < p; }))
            continue;
        for (int i = 0; i < 8; ++i)
        {
