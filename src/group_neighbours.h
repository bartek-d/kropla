 ...
 * forbidden_point. These points are stored in neighbours_list, and also denoted
 * by a 'mask' in 'neighbours'. Moreover, in neighbour_groups we store all
 * group_id's close to these empty points, so that we know which groups can be
 * connected with 'group_id' in one move.
 */
